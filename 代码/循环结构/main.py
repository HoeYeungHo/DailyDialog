...
from Model import get_model, reset_bert_parameters
...
## 参数设置
# Title
title = "Bert_BiLSTM_预训练_全参_alpha_5_" + str(int(time.time()))
print(title)
# 模型参数
...
train_dataloader, valid_dataloader = LoadTrainData(batch_size1 = 8, batch_size2 = 8, device = device, bert=bert)

# 设置模型
# model, times, bestEmotionACC = get_model(modelPath)  # 模型, 模型ID, 最好的ACC
model, times, bestEmotionACC = get_model(
...
elif ResetParameter:  # 不冻结参数且重置参数
    print("重置BERT参数")
    reset_bert_parameters(model.bert_model)
...
for epoch in range(1):
    batch = 0
    for output, emotion, act, SentenceNum, WordNum, batch_size in train_dataloader:
        # 训练阶段
        model.train()
        optimizer.zero_grad()  # 梯度清零
        emotion_hat, act_hat = model(output, SentenceNum, batch_size)

        loss = nn.functional.cross_entropy(emotion_hat.flatten(0, 1), emotion)
        if torch.isnan(loss):
            loss = alpha * nn.functional.cross_entropy(act_hat.flatten(0, 1), act)
        else:
            loss += alpha * nn.functional.cross_entropy(act_hat.flatten(0, 1), act)
        ...
        if batch % 100 == 0 or batch < 50 or (batch < 200 and batch % 5 == 0):
            model.eval()
            first = 1
            for output, emotion, act, SentenceNum, WordNum, batch_size in valid_dataloader:
                emotion_hat, act_hat = model(output, SentenceNum, batch_size)

                if first:
                    emotionALL = emotion[emotion != -100]
                    actALL = act[act != -100]
                    EmotionPredictALL = emotion_hat.flatten(0, 1).argmax(dim = 1)[emotion != -100]
                    AccPredictALL = act_hat.flatten(0, 1).argmax(dim = 1)[act != -100]
                    first = 0
                else:
                    emotionALL = torch.cat((emotionALL, emotion[emotion != -100]), dim=0)
                    actALL = torch.cat((actALL, act[act != -100]), dim=0)
                    EmotionPredictALL = torch.cat((EmotionPredictALL, emotion_hat.flatten(0, 1).argmax(dim = 1)[emotion != -100]), dim=0)
                    AccPredictALL = torch.cat((AccPredictALL, act_hat.flatten(0, 1).argmax(dim = 1)[act != -100]), dim=0)
